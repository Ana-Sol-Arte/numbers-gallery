...
// Mobile/desktop: TAP or CLICK ANYWHERE to cycle modes (Zen → Snow → Water → …)
// Also click the top-left button or press 'M' to cycle; 'R' randomizes the number.
// Smooth transitions: fade-out to black, switch mode, fade-in.
//
...
let pg;                   // offscreen raster of the number
let particles = [];       // zen/water particles
let snow = [];            // snowfall flakes
let lastBuildKey = "";
let startMillis = 0;

// UI
let modeBtn = {x: 12, y: 12, w: 220, h: 36};
...
// ---- Water tuning ----
const TIDE_SECONDS = 7.5;
const WAVE_AMP = 16;
const WAVE_LEN = 120;
const WAVE_SPEED = 0.8;
const CHOP_NOISE_SCALE = 0.004;
const CHOP_STRENGTH = 8;
const WATER_BOB = 3.0;
const WATER_STRETCH = 0.9;
const WATER_FADE = 40;
...
function draw() {
  ...
  const buildKeyNow = DISPLAY_TEXT + "|" + width + "x" + height;
  if (buildKeyNow !== lastBuildKey) {
    buildTextParticles();
    buildSnow();
  }
...
function drawZen() {
  ...
  const tri = ph < 0.5 ? (ph * 2.0) : (2.0 - ph * 2.0);
...
function drawWater() {
  ...
  const tideTri = ph < 0.5 ? (ph * 2.0) : (2.0 - ph * 2.0);
...
function drawHUD() {
  ...
  if (/Mobi|Android/i.test(navigator.userAgent)) {
    ...
    textSize(12);
    fill(FG, 120);
    text('Tap anywhere to switch', width - 12, 12);
    pop();
  }
}
...
function keyTyped() {
  if (key === 'r') {
    ...
    startModeTransition(nextModeAfter(MODE));
  }
}
...
function nextModeAfter(m) {
  return m === "zen" ? "snow" : (m === "snow" ? "water" : "zen");
}

function startModeTransition(toMode) {
